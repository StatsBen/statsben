...
test("Test Parse Commitment Grade", () => {
  ...
  expect(parser.parseCommitmentGrade("VII")).toThrow();
  expect(parser.parseCommitmentGrade(0)).toThrow();
  expect(parser.parseCommitmentGrade(8)).toThrow();
});
...
test("Test Parse Distance", () => {
  ...
  expect(parser.parseDistance(0)).toThrow();
  expect(parser.parseDistance("0")).toThrow();
  expect(parser.parseDistance("poo")).toThrow();
  expect(parser.parseDistance("100fuckyous")).toThrow();
});

test("Test Parse Ice Grade", () => {
  expect(parser.parseIceGrade("WI3")).toEqual("WI3");
  expect(parser.parseIceGrade("AI4  ")).toEqual("AI4");
  expect(parser.parseIceGrade("WI9")).toThrow();
  expect(parser.parseIceGrade("fuckyou")).toThrow();
  expect(parser.parseIceGrade("AI0")).toThrow();
});
...
test("Test Parse YDS", () => {
  ...
  expect(parser.parseYDSGrade("5.11e")).toThrow();
  expect(parser.parseYDSGrade("11b")).toThrow();
  expect(parser.parseYDSGrade(9)).toThrow();
  expect(parser.parseYDSGrade(5.4)).toThrow();
  expect(parser.parseYDSGrade("5.9b")).toThrow();
  expect(parser.parseYDSGrade("5.16")).toThrow();
});
...
test("Test Parse Scrambling Grade", () => {
  ...
  expect(parser.parseScramblingGrade(6)).toThrow();
  expect(parser.parseScramblingGrade(0)).toThrow();
  expect(parser.parseScramblingGrade("0")).toThrow();
  expect(parser.parseScramblingGrade("poopz")).toThrow();
});
...
test("Test Parse Vert", () => {
  ...
  expect(parser.parseVert("1,,300m")).toThrow();
  expect(parser.parseVert("50mi")).toThrow();
  expect(parser.parseVert(0)).toThrow();
  expect(parser.parseVert("poopin yr face")).toThrow();
});
